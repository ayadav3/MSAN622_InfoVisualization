!function(){
	...
	var c1=[-180, 40], c2=[-45, 120], c3=[-5, 160]; //Column positions of labels.
	var colors =["#3366CC", "#DC3912",  "#FF9900","#109618", "#990099", "#0099C6","#1b9e77","#d95f02","#7570b3","#e7298a","#66a61e","#e6ab02","#a6761d","#666666"];
	...
	function visualize(data){
		...
		function calculatePosition(a, s, e, b, m){
			...
			ret.forEach(
				function(d){ 
					...
					d.middle=Math.round(sum+b+d.height/2);
					d.y=s + d.middle - d.percent*(e-s-2*b*a.length)/2;
					d.h= d.percent*(e-s-2*b*a.length);
					d.percent = (total == 0 ? 0 : d.value/total);
					sum+=Math.round(2*b+d.height);
				}
			);
			return ret;
		}
		...
		vis.mainBars = [ 
			calculatePosition( data.data[0].map(function(d){ return Math.round(d3.sum(d));}), 0, height, buffMargin, minHeight),
			calculatePosition( data.data[1].map(function(d){ return Math.round(d3.sum(d));}), 0, height, buffMargin, minHeight)
		];
	...
	function drawHeader(header, id){
		...
		[0,1].forEach(function(d){
			...
			h.append("text").text("Value").attr("x", (c2[d]-15))
				.attr("y", -5).style("fill","grey");
